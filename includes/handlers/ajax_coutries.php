...
for($i = 0; $i < count($login_object['county_hint']); $i++){

    $action = $login_object['county_hint'][$i]['action'];
    $admin_id = $login_object['county_hint'][$i]['admin_id'];
    

    //   GET COLOR DATA STARTS

$sql_customer_request_color = " SELECT *, (SELECT COUNT(*) FROM tbl_fundi_address WHERE tbl_fundi_address.country_id = tbl_countries.country_id) AS total_fundi FROM tbl_countries ORDER BY total_fundi DESC;";
