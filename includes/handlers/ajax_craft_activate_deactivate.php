...
for($i = 0; $i < count($login_object['craft_hint']); $i++){
  ...
  $is_active = $login_object['craft_hint'][$i]['is_active'];
    if(mysqli_query($conn, "UPDATE tbl_fundi_professional SET is_active = '$is_active' WHERE professional_id = '$professional_id';")){
