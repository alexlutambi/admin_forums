...
for($i = 0; $i < count($login_object['default_password_hint']); $i++){
    ...
    $fundi_id = $login_object['default_password_hint'][$i]['fundi_id'];
    $default_password = $login_object['default_password_hint'][$i]['default_password'];
    	$fundi_password = escape($login_object['default_password_hint'][$i]['default_password']);
    $fundi_password = password_hash($fundi_password, PASSWORD_BCRYPT, array('cost'=>12));
     
    if(mysqli_query($conn, "UPDATE tbl_mafundi SET fundi_password = '$fundi_password' WHERE fundi_id = '$fundi_id';")){
