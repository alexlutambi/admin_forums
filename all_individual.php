...
$sql_individual_all_total_results = "SELECT * FROM tbl_wateja ORDER BY mteja_id DESC;";		
	 $per_page_start = 10;
  $page_start = 1;
	  $result_individual_all_total_results = mysqli_query($conn, $sql_individual_all_total_results);
	  $number_of_results = mysqli_num_rows($result_individual_all_total_results);
...
$remained_pages = $number_of_pages - $page_start;
	$sql_individual_results = "SELECT * FROM tbl_wateja ORDER BY mteja_id DESC LIMIT ".$this_page_first_result.','.$per_page_start.";";
			...
			if($user['username'] != $row['username']) {
				...
				if(isset($_POST[$row['username']])) {
					...
					<div class='all-user-single-profile-pic'>
						...
						<a href='profile.php?profile_username=" . $row['username'] ."'><img id='individual-list-img-" . $row['mteja_id'] . "' class='profile-available individual-list-img individual-list-img-" . $row['mteja_id'] . "' mteja_id='" . $row['mteja_id'] . "' mteja_profile='" . $row['mteja_profile'] . "' src='../mteja/mteja_profile/mteja_thumb_color/" . $row['mteja_thumb_color'] ."' style='height: 100px;'></a>
					</div>
...
echo "<div id='fundi-activation-deactivation-container-";
          ...
          echo "' account_type='mteja' class='btn-all-activate-deactivate danger' >DeActivate</button>";
 				}else{
          			echo "<button id='btn-all-activate-deactivate-";
					echo $row['mteja_id'];
					echo "' mteja_id='";
          ...
          echo "' account_type='mteja' class='btn-all-activate-deactivate success' >Activate</button>";
        }
...
var product_hint = {};
product_hint["action"] = "all_individual";
product_hint["per_page"] = 10;
product_hint["page"] = next_page;
product_hints.push(product_hint);

		var action_quality = "includes/handlers/ajax_all_individual.php?page=" + next_page;
...
var result = xhr_quality.responseText;
 console.log('Result_all_individual: ' + result);
			...
			get_paging_all_individual(username, main_column, result);
...
xhr_quality.send(JSON.stringify({"all_hint" : product_hints}));
      ...
      function get_paging_all_individual(username, container, result){
    var li_success_list = selectedPagingAllIndividual(username, result);
...
var individual_list_container = document.getElementById(btn_activate_deactivate_id);
individual_list_container.addEventListener('click', function(){
...
var mteja_status = product_id.getAttribute("mteja_status");
 ...
 function appendToDiv(div, new_html){
    ...
    function selectedPagingAllIndividual(username, result){
...
var server_link = body_main.getAttribute("server_link");
 ...
 load_more.setAttribute("data-page", page);
	...
	var mteja_full_name = json_result[k].mteja_full_name;
	var mteja_profile = json_result[k].mteja_profile;
	var mteja_thumb_color = json_result[k].mteja_thumb_color;
			...
			output += '<div class="all-user-single-container">';
					...
					output += '<div class="all-user-single-profile-pic">';
						output += '<a href="profile.php?profile_username=';
						 output += username;
						 output += '"><img id="individual-list-img-';
						 output += mteja_id;
						 output += '" class="profile-available individual-list-img individual-list-img-';
						  output += mteja_id;
						  output += '" mteja_id="';
						   output += mteja_id;
						   output += '" mteja_profile="';
						    output += mteja_profile;
							output += '" src="../mteja/mteja_profile/mteja_thumb_color/';
							 output += mteja_thumb_color;
          ...
          output += '" account_type="mteja" class="btn-all-activate-deactivate danger" >DeActivate</button>';
 				}else{
          			output += '<button id="btn-all-activate-deactivate-';
					output += mteja_id;
					output += '" mteja_id="';
          ...
          output += '" account_type="mteja" class="btn-all-activate-deactivate success" >Activate</button>';
        }
